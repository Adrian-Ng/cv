...
\begin{document}
	...
	{	\tiny (\href{https://adrian.ng/java/var/}{https://adrian.ng/java/var/}) \quad (\href{https://github.com/Adrian-Ng/VaR}{https://github.com/Adrian-Ng/VaR})
		...
		In addition, the following approaches to estimating market variance- are implemented: \textit{Equal Weighted}, \textit{Exponentially Weighted Moving Average}, and \textit{GARCH(1,1)}.
	}			
	\resumeItemListStart		
	\resumeItem{Object Oriented Design}
	{
		As we have a number of approaches to estimating both \textit{VaR} and \textit{variance}, abstract methods \textit{getVariance()} and \textit{getVaR()} are defined in abstract classes but implemented in child classes. Instantiation of these child classes is handled via the \textit{factory method}.
	}	
	\resumeItem{Concurrency}
	{
		The \textit{Monte Carlo} approach generates a large number of random walks, which can take a long time to fully execute. Therefore execution is parallelized using \textit{java.util.concurrent} for which \textit{Callable} and \textit{Future} lambdas are implemented.
	}			
	\resumeItem{Data Ingress}
	{
		Real-world market data is sourced using the \textit{Yahoo Finance API}, which returns an iterable collection of \textit{BigDecimal}. These daily closing prices are transformed into daily price changes of type \textit{Double[]}. Thus sacrificing accuracy for computational performance. 
		\newline
		The hypothetical portfolio with a number of stocks and options was defined locally across a number of text-files.
	}		
	\resumeItemListEnd	
	...
	\resumeSubItem{OptionPricer}	
	{
		...
		\newline
		\small
		This project implements three approaches to estimating option prices in Java: \textit{Monte Carlo simulation}, \textit{Black-Scholes equations}, and\textit{Binomial Trees}. Option prices for each of these approaches were retrieved through the interface methods \textit{getCall()} and \textit{getPut()}.		
	...
	{	
		...
		\newline
		These applications extended \textit{Mapper} and \textit{Reduce}. Methods \textit{map()} and \textit{reduce()} were overridden with bespoke implementation.
		\newline 
	...
	\resumeSubItem{Mini Project: Java 8 Streams with financial data}	
	{
		...
		\newline
		\small
		A small exercise involving the use of \textit{Java 8 Streams}. Processing real-world financial data to return mean and variance by defining input functinos to \textit{map()} and \textit{reduce()} methods.
	}
...
%	}		
	...
	{
		This project involved the creation of a number of automated processes to merge GDPR preference data with the analytical database. To accomplish this, SQL DML such as \textit{merge} were utilised.
	}
	...
	{
		Built an automated segmentation process using SQL stored procedures for other members of the team to utilise. Recipient data were imported via \textit{BULK INSERT}, stored in database tables and indexed (clustered). Segmentation data was output and linked to HTML content to be broadcast to recipients.
	}

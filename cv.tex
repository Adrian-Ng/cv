...
%		While there was nothing I could do about the structure of the database, I was able to replace the entire \texttt{Informatica} process with a single stored procedure that simply queryied the remote database with an \texttt{OPENQUERY} and a \texttt{MERGE}. Automating this would have been a matter of using \textit{SQL Sever Information Services} but for some reason it would just not work with SSMS 2008. By they time the database had been upgraded to a better version, I was long gone.
		...
		A long-term project involving many phases: data-discovery, pipeline building, data aggregation, dashboarding.
		
	\resumeItemListStart
		
		\resumeItem{Data Pipeline}{}
		{
			Built a data pipeline which replaced existing \texttt{Informatica} solution to \texttt{OPENQUERY} remote servers and \texttt{MERGE} into local SQL server. The resulting solution was more efficient and was encapsulated in a stored procedure. 
			\newline
		}	
		
		\resumeItem{Salesforce Data}{}
		{
			Phone sales data also came from \textit{Salesforce}. I liaised with \texttt{KORE} to get this data over to us.
			\newline
		}	
		\resumeItem{Data Hygiene}{}
		{
			Liaised with Major League Soccer who provided customer demographic data.
			\newline
